...
class M_kelolakegiatan extends CI_Model{
	...
	function jumlahKegiatanPerbidang()
	{
		...
		$hariini=date("d-m-Y");
		$bulanini = substr($hariini,3,2);
		$query2 = $this->db->query("SELECT substring(id_jeniskegiatan,1,4) as bidang, sum(nilai_total) as nilai, count(id_jeniskegiatan) jmlh, sum(nilai_total)/count(id_jeniskegiatan) as rata_rata FROM `t_kegiatan` where nilai_total <> ''  and target <> '0' group by bidang");
	...
	function cobaJumlahKegiatanPerbidang()
	{
		...
		$hariini=date("d-m-Y");
		$bulanini = substr($hariini,3,2);
		return $this->db->query("SELECT substring(id_jeniskegiatan,1,4) as id_bidang, sum(nilai_total) as nilai, count(id_jeniskegiatan) jmlh, sum(nilai_total)/count(id_jeniskegiatan) as rata_rata FROM `t_kegiatan` where nilai_total <> '' and target <> '0' group by id_bidang")->result();
	}
	...
	function jumlahKegiatanPerkabkot()
	{
		...
		$query5 = $this->db->query("SELECT t.id_kab, k.nama_kab,COUNT(t.id_jeniskegiatan) AS total_kegiatan
			...
			WHERE MONTH(m.batas_waktu)='10' and t.target <> '0'
			group by t.id_kab");
