		...
		else
		{
			...
			{*/
				...
				foreach ($wilayah as $row)
				{
					$kode_kab = $row->id_kab;
					$data_kab = addslashes($this->input->post('_'.$kode_kab));
					$this->db->query("INSERT INTO t_kegiatan VALUES ('$id_jeniskegiatan', '$kode_kab','$data_kab',0,'','2','','','','','','','','','','')");
				}
				$this->db->query("INSERT INTO m_jeniskegiatan VALUES ('$id_jeniskegiatan', '$nama_kegiatan', $targetkabkumulatif,0, '$satuan', '$batas_waktu','$dasar_surat')");
...
public function kegiatan()
	{
		...
		else
		{
			...
			{
			$bidangku = $this->session->userdata('admin_user');
			$a['data']		= $this->db->query("SELECT k.*,u.unitkerja,s.satuan as nama_satuan FROM m_jeniskegiatan k left join m_unitkerja u on substring(k.id_jeniskegiatan,1,5) = u.id_unitkerja  left join m_satuan s on k.satuan=s.id_satuan  where YEAR(k.batas_waktu)='$tahun' and substring(u.id_unitkerja,1,4) = '$bidangku' order by k.batas_waktu asc")->result();
			}
			else
			{
	...
	public function unitkerjakabkota()
	{
		...
		if ($mau_ke == "pilih_kegiatan")
		{
			if($id_bulan == '00')
			{
				...
				{
					$a['data']			= $this->db->query("SELECT k.*,u.unitkerja,t.id_kab,s.satuan as nama_satuan FROM m_jeniskegiatan k left join m_unitkerja u on substring(k.id_jeniskegiatan,1,5) = u.id_unitkerja left join t_kegiatan t on k.id_jeniskegiatan=t.id_jeniskegiatan  left join m_satuan s on k.satuan=s.id_satuan  where YEAR(k.batas_waktu)='$tahun' and t.id_kab='$id_kabkota' and t.target <> '0' order by k.batas_waktu asc")->result();
					$a['page']			= "l_kegiatanunitkerjakabkota";	
				}
			...
			else
			{
				if($id_kabkota == '3300')
				{
				$a['data']			= $this->db->query("SELECT k.*,u.unitkerja,s.satuan as nama_satuan FROM m_jeniskegiatan k left join m_unitkerja u on substring(k.id_jeniskegiatan,1,5) = u.id_unitkerja left join m_satuan s on k.satuan=s.id_satuan   where YEAR(k.batas_waktu)='$tahun' and MONTH(k.batas_waktu)='$id_bulan' order by k.batas_waktu asc")->result();
				$a['page']			= "l_kegiatanunitkerjakabkota";	
				}
				else
				{
					$a['data']			= $this->db->query("SELECT k.*,u.unitkerja,t.id_kab,s.satuan as nama_satuan FROM m_jeniskegiatan k left join m_unitkerja u on substring(k.id_jeniskegiatan,1,5) = u.id_unitkerja left join t_kegiatan t on k.id_jeniskegiatan=t.id_jeniskegiatan  left join m_satuan s on k.satuan=s.id_satuan  where YEAR(k.batas_waktu)='$tahun' and t.id_kab='$id_kabkota' and MONTH(k.batas_waktu)='$id_bulan'  and t.target <> '0' order by k.batas_waktu asc")->result();
					$a['page']			= "l_kegiatanunitkerjakabkota";	
				}
			}
		}
